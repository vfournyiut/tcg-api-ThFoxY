...
async function main() {
  ...
  const redDeck = await prisma.deck.findMany({
    where: { name: "red's Starter Deck" },
  });
  const blueDeck = await prisma.deck.findMany({
    where: { name: "blue's Starter Deck" },
  });
  ...
  const redDeckCards = await prisma.deckCard.findMany({
    where: { deckId: redUser.id },
  });
  const blueDeckCards = await prisma.deckCard.findMany({
    where: { deckId: blueUser.id },
  });
